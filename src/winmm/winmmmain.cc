...
void Init()
{
	...
	if (nt->OptionalHeader.AddressOfEntryPoint != 0x0EC020)		// Check the swos-port-release executable version
	{
    if (MessageBoxA(NULL, "Not valid version of SWOS executable\nContinue anyway?", "ASI Loader v0.3.5b", MB_YESNO + MB_ICONASTERISK) == IDYES)
    {
		  LoadPlugin();
    }
	  else
	  {
		  MessageBoxA(NULL, "Unsupported version of SWOS, or not SWOS-Win32-Port.", "ASI Loader v0.3.5b", MB_ICONWARNING + MB_OK);
	  }
  }
  else
  {
